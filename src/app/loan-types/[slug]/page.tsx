...
export async function generateStaticParams() {
  return TIER2_PAGES.filter(p => p.type === "loan-types").map((page) => ({
    slug: page.slug,
  }));
}
...
export async function generateMetadata(props: {
  ...
  const pageData = getTier2PageBySlug(params.slug, "loan-types");
  ...
  return generateSeoMetadata({
    ...
    path: `/loan-types/${params.slug}`,
  });
}

export default async function LoanTypePage(props: {
  params: Promise<{ slug: string }>;
}) {
  const params = await props.params;
  const pageData = getTier2PageBySlug(params.slug, "loan-types");
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: "Home", item: "/" },
    { name: "Loan Types", item: "/loan-types" },
    { name: pageData.h1, item: `/loan-types/${params.slug}` },
  ]);
