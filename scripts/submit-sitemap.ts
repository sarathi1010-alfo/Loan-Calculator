...
async function submitSitemap() {
  ...
  if (!fs.existsSync(dataPath)) {
    console.error("❌ Error: seo-pages.json not found. Run entity generator first.");
    process.exit(1);
  }
  ...
  const clusterTypes = Array.from(new Set(entities.map((e: any) => e.loanType)));
  const sitemaps = clusterTypes.map(type => `${SITE_URL}/sitemap-${type}-loan-calculator.xml`);
